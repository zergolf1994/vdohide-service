    ...
    try {
        ...
        for (const storage of storages) {
            ...
            if (sourceSlots <= 0) {
                await StorageModel.updateOne(
                    { _id: storage._id },
                    { $set: { drainState: StorageDrainState.BLOCKED, drainError: "Transfer worker is offline" } }
                );
                continue;
            }
            ...
            if (!temp) {
                await StorageModel.updateOne(
                    { _id: storage._id },
                    { $set: { drainState: StorageDrainState.BLOCKED, drainError: "No temp storage available" } }
                );
                continue;
            }
            ...
            enqueued += await enqueueEvacuate(
                storage,
                String(temp._id),
                Math.max(0, availableSlots - cleanupEnqueued)
            );

            const [remainingMedia, openJobs, failedJobs, remainingMigration] = await Promise.all([
                MediaModel.countDocuments({ storageId: storage._id, deletedAt: { $exists: false } }),
